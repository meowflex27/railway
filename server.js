...
async function handleMovieboxFetch(tmdbId, isTV = false, season = 0, episode = 0) {
  ...
  if (!subjectId) throw new Error(`❌ subjectId not found for "${title}"`);
  ...
  const detailsUrl = detailPath
    ...
    : `https://moviebox.ph/?id=${subjectId}`; // fallback
